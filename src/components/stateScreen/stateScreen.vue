    ...
    <div class="left">
      ...
      <div class="manage-table">
        <el-table :data="tableData" style="width: 100%">
          <el-table-column prop="seat" label="位置"></el-table-column>
          <el-table-column prop="goods" label="商品"></el-table-column>
          <el-table-column prop="num" label="数量"></el-table-column>
          <el-table-column prop="operation" label="操作">
            <template slot-scope="scope">
              <el-button
                @click.native.prevent="handleClick(scope.row,scope.$index)"
                type="text"
                size="small"
              >完成</el-button>
            </template>
    ...
    <div class="center">
      ...
      <div class="IndentBox">
        <div class="indent" v-for="(val,index) in OrderListB" :key="index">
          <div class="info">
            <div class="one">
              <p class="name">{{val.address.name}}</p>
              <span class="haoma">{{val.address.mobile}}</span>
            </div>
            <div class="two">
              <p class="ear">{{val.address.area}}</p>
            </div>
          ...
          <div class="faishBox">
            <div class="finsh">
              <span class="tang">
                <el-radio v-model="val.radio" label="1">堂客</el-radio>
              </span>
              <span
                class="wan"
                :class="val.radio == 1?'active':''"
                v-on:click="TKwan(index,val.id,val.radio)"
              >完成</span>
            </div>
            <div class="yes">
              <span class="hao">
                <el-radio v-model="val.radio" label="2">订单号</el-radio>
              </span>
              <input
                type="text"
                class="input"
                :disabled="val.radio == 1? true : false"
                v-model="val.OrderHao"
              >
              <span
                class="que"
                :class="val.radio == 2?'active':''"
                v-on:click="SendHuo(index,val.id,val.radio,val.OrderHao)"
              >确认发货</span>
            </div>
          </div>
          <div>
            ...
            <div class="yiwan" style="display: none;">
              <span class="tang">
                ...
                <span class="dingdan">156456454132132</span>
              </span>
...
export default {
  ...
  methods: {
    handleClick(row, index) {
      ...
      this.$http
        .put(this.$conf.env.OrderListToady + row.order + "/", OrderID)
        .then(res => {
          if (res.status === 200) {
            console.log();
    ...
    TKwan(index, id, raiods) {
      ...
      this.$http
        .put(this.$conf.env.OrderPut + id + "/", DataJson)
        .then(res => {
          if (res.status === 200) {
    ...
    SendTeaList() {
      ...
      this.$http
        .post(this.$conf.env.SendListTea, Obj)
        .then(res => {
          if (res.status === 201) {
    ...
    changeXi(id, index) {
      ...
      let params = {};
      this.$http
        .put(this.$conf.env.ChangeXiStatus + id + "/", params)
        .then(res => {
          if (res.status === 200) {
    ...
    OrderList() {
      ...
      this.$http
        .get(this.$conf.env.TodayListTea)
        .then(res => {
          if (res.status === 200) {
            console.log(res.data);
      ...
      this.$http
        .get(this.$conf.env.OrderListToady)
        .then(res => {
          if (res.status === 200) {
            console.log(res.data);
    ...
    websocketonmessage(e) {
      ...
      if (ObjData.type.length == 2) {
        this.tableData.push(ObjData.info);
  ...
  .center {
    ...
    .IndentBox {
      ...
      .indent {
        ...
        border-radius: 5px;
        margin-left: 0.18rem;
        padding: 0 0.21rem 0.2rem 0.12rem;
        font-size: 0.2rem;
